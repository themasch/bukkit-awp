...
/**
 * a rather simple and dump implementation of the repository pattern to manage
 * warp points in AWP
 * 
 * This class is used to retrieve, update and store warp points with an easy to
 * use interface.
 * 
 * @package AWP
...
class WarpPointManager
{
    ...
    private Map<String, Map<String, WarpPoint>> warpPoints;
    ...
    public WarpPointManager(ConfigurationSection config)
    {
        ...
        for (String player : config.getKeys(false)) {
            ...
            warpPoints.put(player.toLowerCase(), temp);

            List<Map<?, ?>> points = config.getMapList(player);

            for (Map<?, ?> map : points) {
                WarpPoint point = WarpPoint
                        .fromConfig((Map<String, String>) map);
                temp.put(point.getName().toLowerCase(), point);
            }
        }
    }
     ...
     * 
     * @param sender
     *            Player who searches for waypoints
     * @param input
     *            Search string
     * @param ignorePermissions
     *            defines if private waypoints should be hidden if access isn't
     *            granted
     * @return the first found warp point. Results owned be the sender are
     *         sorted above others
     */
    public WarpPoint findFirst(OfflinePlayer sender, String input,
     ...
     * 
     * Search string support glob style matching: "bla.*" matches "bla.baz" and
     * "bla.foo" but not "bla" nor "blubb" "?oo" matches "foo" and "boo"
     * "home.{mine,yours}" matches only "home.mine" or "home.yours"
     * 
     * Additionally the input string might be prefixed by the name (or part of
     * the name) of the warp points owner. If the substring until the first dot
     * (".") matches a known players name, his warp points are searched using
     ...
     * features the players own warp points - if existing - on the top of the
     * list. Warp points of other players follow.
     * 
     ...
     * creating and changing warp points and should therefore not cause any
     * trouble.
     * 
     * @param sender
     *            Player who searches for waypoints
     ...
     *            granted
     */
    public List<WarpPoint> find(final OfflinePlayer sender, String input,
            boolean ignorePermissions)
    {
        ...
        if (points == null) {
            ...
            for (String plName : warpPoints.keySet()) {
                if (plName.equals(sender.getName())) {
                    continue;
                }
        ...
        Predicate<WarpPoint> p = new Predicate<WarpPoint>()
        {

            @Override
            public boolean apply(@Nullable WarpPoint wp)
            {
                return wp.isAllowed(sender);
            }

        };

        return new ArrayList<>(Collections2.filter(points, p));

        // List<WarpPoint> filtered = new ArrayList<>();
    ...
    /**
     * Finds a list of warp points matching the given input string that are
     * owned by the owner.
     * 
     * @param owner
     *            the Owners name
     * @param input
     *            Search string. See find() for details on its format.
     */
    ...
    {
        return findOwnedBy(owner.getName(), input);
    }
    ...
    public boolean setVisibility(OfflinePlayer owner, String name,
            boolean visible)
    {
        return false;
    }
    ...
    public boolean invite(OfflinePlayer owner, String name, Set<String> players)
    {
        ...
        for (WarpPoint wp : wpList) {
            wp.invite(invited);
        }
        return false;
    }

    public boolean uninvite(OfflinePlayer owner, String name,
            Set<String> players)
    {
        List<WarpPoint> wpList = findOwnedBy(owner, name);
        Set<OfflinePlayer> invited = findPlayers(players);
    ...
    protected OfflinePlayer findPlayerByName(String name)
    {
        ...
        List<Player> candidates = Bukkit.getServer().matchPlayer(name);
        return candidates.isEmpty() ? null : candidates.get(0);
    }

    /**
     * shamelessly pirated from
     * https://stackoverflow.com/questions/1247772/is-there
     * -an-equivalent-of-java-util-regex-for-glob-type-patterns thx interwebs!
     */
